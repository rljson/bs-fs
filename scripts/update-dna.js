...
const filesToBeDeleted = [
  ...
  'doc/workflows/update-from-bs-fs.md',
];

const templateRepo = 'https://github.com/rljson/bs-fs.git';
const localRepoPath = path.resolve(__dirname, '../../bs-fs');
...
function replaceBsFs() {
  ...
  for (const file of replaceFiles) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      content = content.replace(/bs-fs/g, projectName);
